...
public class SysUserServiceImpl extends BaseServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
    ...
    public PageResult<SysUserVO> pageByShop(SysUserQuery query) {
        ...
        IPage<SysUserEntity> page = baseMapper.selectPage(getPage(query), wrapper);
        List<SysUserVO> sysUserVOS = SysUserConvert.INSTANCE.convertList(page.getRecords());
    ...
    private LambdaQueryWrapper<SysUserEntity> getWrapper(SysUserQuery query) {
        ...
        wrapper.like(ObjectUtil.isNotEmpty(query.getUsername()), SysUserEntity::getUsername, query.getUsername());
        wrapper.eq(ObjectUtil.isNotEmpty(query.getMobile()), SysUserEntity::getMobile, query.getMobile());
        wrapper.eq(ObjectUtil.isNotEmpty(query.getGender()), SysUserEntity::getStatus, query.getGender());
        UserDetail user = SecurityUser.getUser();
        if (user != null && SuperAdminEnum.YES.getValue().equals(user.getSuperAdmin())) {
            wrapper.ne(SysUserEntity::getSuperAdmin, Constant.SUPER_ADMIN);
            return wrapper;
        }else if(user != null && SuperAdminEnum.NO.getValue().equals(user.getSuperAdmin())){
            wrapper.and(wrapper1 -> wrapper1
                            .eq(ObjectUtil.isNotNull(user.getOrgId()), SysUserEntity::getOrgId, user.getOrgId())
                            .ne(ObjectUtil.isNotNull(user.getId()), SysUserEntity::getId, user.getId()))
                    .ne(SysUserEntity::getSuperAdmin, Constant.SUPER_ADMIN);

            return wrapper;
        }else{
            throw new ServerException("当前未登录用户");
        }
    }
