...
public class SysAuthServiceImpl implements SysAuthService {
    ...
    @Override
    public SysUserTokenVO loginByWxUser(SysAccountLoginVO login) {
        ...
        // 用户信息
        UserDetail user = (UserDetail) authentication.getPrincipal();
            if(ObjectUtil.isNotNull(login.getOrgId())){
                if(!Objects.equals(user.getOrgId(), login.getOrgId())){
                    throw new ServerException("门店错误");
                }
            }else{
                throw new ServerException("未选择门店");
            }
